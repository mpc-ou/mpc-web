...
export function DashboardChart({ data }: { data: ChartData[] }) {
  ...
  return (
    <div className='rounded-xl border border-border bg-background p-6 shadow-sm'>
      <div className='mb-6'>
        <h3 className='font-semibold text-base text-foreground tracking-tight'>Content activity</h3>
        <p className='text-muted-foreground text-xs'>Posts by type (Blog, Event, Achievement) over the last 6 months</p>
      </div>
      <div className='h-[320px] w-full'>
        <ResponsiveContainer height='100%' width='100%'>
          <BarChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            ...
            <Bar dataKey='achievement' fill='#6366f1' maxBarSize={24} name='Achievement' radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>
