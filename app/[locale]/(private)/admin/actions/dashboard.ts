...
export const adminGetDashboardStats = async () =>
  handleErrorServerWithAuth({
    cb: async ({ user }) => {
      ...
      const [members, events, posts, announcements, sponsors, departments, faqItems, galleryImages, achievements, projects] = await Promise.all(
        [
          prisma.member.count(),
          prisma.event.count(),
          prisma.post.count(),
      ...
      return { members, events, posts, announcements, sponsors, departments, faqItems, galleryImages, achievements, projects };
    }
  });
