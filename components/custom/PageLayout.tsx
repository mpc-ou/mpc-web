...
import { getActiveAnnouncement, getHeaderProfile } from "@/app/[locale]/actions";
...
const PageLayout = async ({ children }: { children: ReactNode }) => {
  const [announcementResult, profileResult] = await Promise.allSettled([getActiveAnnouncement(), getHeaderProfile()]);

  for (const result of [announcementResult, profileResult]) {
