...
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: member card front SVG layout
function MemberCardFront({ data, locale, isHorizontal, frontRef, t }: CardFrontProps) {
  return (
    <div className='absolute inset-0 bg-white' ref={frontRef}>
      ...
      {/* biome-ignore lint/performance/noImgElement: export */}
      ...
      >
        ...
        >
            ...
            <div className='relative'>
              <div className='relative z-10 h-48 w-48 overflow-hidden rounded-full border-[6px] border-white bg-slate-100 shadow-[0_10px_30px_rgba(234,88,12,0.15)]'>
                {data.avatar ? (
                  // biome-ignore lint/performance/noImgElement: export
                  // biome-ignore lint/correctness/useImageSize: sized via CSS
                  <img alt='Avatar' className='h-full w-full object-cover' crossOrigin='anonymous' src={data.avatar} />
                ) : (
          ...
          <div className='mt-2 w-full text-center'>
            ...
            <div className='mt-3 font-bold font-mono text-slate-600 text-sm tracking-[0.2em]'>{data.studentId}</div>
          </div>
        </div>
        ...
        >
          ...
          <div className={`${isHorizontal ? "mt-2 text-left" : "text-center"} w-full`}>
          ...
          <div className={`${isHorizontal ? "mb-4 text-left" : "my-4 text-center"} w-full`}>
            <h3
            ...
            >
              ...
              <span className='block'>{locale === "vi" ? data.firstName : data.lastName}</span>
            </h3>
          </div>
          ...
          >
            <div className='flex flex-col border-orange-500/20 border-b pb-2'>
              ...
              <span className='font-bold text-lg text-slate-800 leading-none'>{t("IT")}</span>
            </div>
            ...
            <div className='flex w-full justify-between gap-4'>
              <div className='flex w-1/2 flex-col border-orange-500/20 border-b pb-2'>
                ...
                <span className='font-bold text-lg text-slate-800 leading-none'>{data.dob}</span>
              </div>
              <div className='flex w-1/2 flex-col border-orange-500/20 border-b pb-2'>
                <span className='mb-1 font-black text-[#ea580c] text-[10px] uppercase leading-none tracking-widest'>
                  {t("joinedYear")}
                </span>
                <span className='font-bold text-lg text-slate-800 leading-none'>{data.joinedYear}</span>
              </div>
            </div>
            ...
            <div className='flex flex-col border-orange-500/20 border-b pb-2'>
              ...
              <span className='font-bold text-lg text-slate-800 leading-none'>{data.department}</span>
            </div>
          </div>
        ...
        <div className={`flex flex-col items-center justify-center ${isHorizontal ? "mt-8" : "mr-8"} gap-8`}>
          ...
          <div className='space-y-4 text-center'>
            ...
            <div className='mt-6 space-y-1'>
              <p className='font-mono text-slate-400 text-xs tracking-widest'>EST. 2015</p>
...
export function MemberCardClient({ data, locale }: MemberCardProps) {
  ...
  const handleDownload = useCallback(
    async (ref: React.RefObject<HTMLDivElement | null>, filename: string, setDownloading: (val: boolean) => void) => {
      if (!ref.current) {
        return;
      }
      setDownloading(true);
    ...
    <div className='mx-auto flex w-full max-w-7xl flex-col items-center px-4 py-8'>
      ...
      <div className='mb-10 flex w-full max-w-2xl flex-wrap justify-center gap-4 rounded-2xl bg-slate-100 p-2 dark:bg-slate-900'>
        ...
        <Button
          ...
          onClick={() => handleDownload(frontRef, "MPC_Card_Front.png", setIsDownloadingFront)}
          variant='outline'
        >
        ...
        <Button
          ...
          onClick={() => handleDownload(backRef, "MPC_Card_Back.png", setIsDownloadingBack)}
          variant='outline'
        >
          <Download className='mr-2 h-4 w-4' />
      ...
      <div className='flex w-full flex-col items-center gap-12 pb-16' ref={containerRef}>
        <div className='flex w-full justify-center transition-all duration-300' style={{ height: cardHeight * scale }}>
          <div
          ...
          >
            <MemberCardFront data={data} frontRef={frontRef} isHorizontal={isHorizontal} locale={locale} t={t} />
          </div>
        </div>
        ...
        <div className='flex w-full justify-center transition-all duration-300' style={{ height: cardHeight * scale }}>
          ...
          >
            <MemberCardBack backRef={backRef} isHorizontal={isHorizontal} />
          </div>
        </div>
      </div>
    </div>
