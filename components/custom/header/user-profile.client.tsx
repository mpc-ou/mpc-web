    ...
    <>
      ...
      <Dialog onOpenChange={setShowLogoutDialog} open={showLogoutDialog}>
        <DialogContent className='sm:max-w-100'>
          <DialogHeader>
            <DialogTitle>{tUserMenu("logoutDialog.title")}</DialogTitle>
            <DialogDescription>{tUserMenu("logoutDialog.description")}</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button onClick={() => setShowLogoutDialog(false)} type='button' variant='outline'>
              {tUserMenu("logoutDialog.cancel")}
            </Button>
            <Button onClick={handleLogout} type='button' variant='destructive'>
              <LogOut className='mr-2 h-4 w-4' />
              {tUserMenu("logoutDialog.confirm")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
