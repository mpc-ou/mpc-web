...
import { ChevronLeft, ChevronRight, CircleDot, FolderGit2, Trophy } from "lucide-react";
...
export function LeadershipCarouselClient({ leaders }: { leaders: Leader[] }) {
  ...
  useEffect(() => {
    if (scrollContainerRef.current) {
      const activeElement = scrollContainerRef.current.children[currentIndex] as HTMLElement | undefined;
      if (activeElement) {
  ...
  if (leaders.length === 0) {
    return <div className='py-20 text-center text-muted-foreground'>{t("leadership.emptyData")}</div>;
  }
  ...
  return (
    <section className='relative flex h-[80vh] min-h-[600px] w-full flex-col overflow-hidden border-border border-y bg-black'>
      {leaders.map((leader, index) => (
        ...
        >
          <Image
            alt={getFullName(leader.member.firstName, leader.member.lastName, locale)}
          ...
          >
            ...
            <div className='mb-6 flex flex-col-reverse gap-6 md:flex-row md:items-end md:justify-between'>
              <div>
                ...
                >
                  {getFullName(activeLeader.member.firstName, activeLeader.member.lastName, locale)}
                </h2>
                <div className='mt-4 flex flex-wrap gap-4 font-medium text-sm'>
                  {(activeLeader.member._count?.achievements ?? 0) > 0 && (
                    <div className='flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-1.5 text-white backdrop-blur-sm'>
                      ...
                      <span>
                        {activeLeader.member._count?.achievements}{" "}
                  ...
                  {(activeLeader.member._count?.projects ?? 0) > 0 && (
                    <div className='flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-1.5 text-white backdrop-blur-sm'>
                      <FolderGit2 className='h-4 w-4 text-blue-400' />
                      <span>
                        {activeLeader.member._count?.projects}{" "}
                        {t("leadership.projectsCount", {
                          defaultValue: "Dự án"
                        })}
                      </span>
                    </div>
            ...
            <div className='max-w-xl text-white/90'>
              ...
              <div className='flex flex-col gap-4'>
                {activeLeader.roles.map((role) => {
                  ...
                  const endYear = role.endAt ? new Date(role.endAt).getFullYear() : t("leadership.present");

                  return (
                    <div className='flex items-start gap-4 border-white/20 border-l pl-4' key={role.id}>
            ...
            <div className='mt-8'>
              ...
              >
                <Link href={`/members/${activeLeader.member.slug}`}>
                  <CircleDot className='mr-2 h-4 w-4' /> {t("leadership.viewProfile")}
                </Link>
              </Button>
            </div>
          </div>
