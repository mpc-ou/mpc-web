...
const GALLERY_CATEGORY_DETAILS: Record<
  string,
  { title: string; description: string; icon: React.ComponentType<{ className?: string }>; bgGradient: string }
> = {
  common: {
    title: "Trang chủ",
  ...
  const handleSeedDefaults = async () => {
    ...
    await handleErrorClient({
      ...
      onSuccess: ({ data }) => {
        ...
        toast({
          title: "Thành công",
          description: `Đã nạp thành công ${count} ảnh mặc định vào database!`
        });
        startTransition(() => {
          router.refresh();
        });
                ...
                >
                  <div>
                    <div className='mb-4 flex items-center justify-between'>
                      <div className='flex h-11 w-11 items-center justify-center rounded-xl bg-background/80 text-primary shadow-xs backdrop-blur-xs'>
          ...
          {selectedType && GALLERY_CATEGORY_DETAILS[selectedType] && (
            <div className='mb-2 rounded-2xl border border-border bg-card p-6 shadow-xs'>
              <div className='flex items-center gap-4'>
                {(() => {
                  const ActiveIcon = GALLERY_CATEGORY_DETAILS[selectedType].icon;
                  return (
