...
export function GoldBoard({
  ...
  const sorted = [...members]
    ...
    .sort(
      (a, b) =>
        (b.member._count?.achievementEntries ?? 0) -
        (a.member._count?.achievementEntries ?? 0),
    );
    ...
    >
      <div className="relative">
        <Avatar className="h-16 w-16 border-2 border-background shadow-md ring-2 ring-amber-500/30 transition-all group-hover:ring-amber-500/60">
          ...
          <AvatarFallback className="bg-amber-500/10 font-bold text-amber-600">
            {initials}
          </AvatarFallback>
        </Avatar>
        {achievementCount > 0 && (
