  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const typeColors: Record<string, string> = {
        BLOG: "bg-info/10 text-info border-info/20",
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className='flex items-center gap-1.5'>
          <span
            className={sl === "VI" ? "rounded-xs opacity-100 ring-1 ring-primary/40" : "opacity-35 grayscale"}
            title={hasVi ? "Có bản tiếng Việt" : "Chưa có bản tiếng Việt"}
          >
            <FlagViIcon className='h-3.5 w-4 rounded-xs' />
          </span>
          <span className='text-[10px] text-muted-foreground'>/</span>
          <span
            className={sl === "EN" ? "rounded-xs opacity-100 ring-1 ring-primary/40" : "opacity-35 grayscale"}
            title={hasEn ? "Has English version" : "No English version"}
          >
            <FlagEnIcon className='h-3.5 w-4 rounded-xs' />
          </span>
        </div>
      );
    },
