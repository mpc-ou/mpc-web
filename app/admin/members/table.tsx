...
import { ChevronDown, Database, Download, RefreshCw, Search, Settings2, Trash2, UserCheck } from "lucide-react";
...
export function MembersDataTable({
  ...
  const handleView = useCallback((member: MemberRow) => {
  ...
  const handleDelete = useCallback(
    ...
    [confirm, router, handleErrorClient]
  );
  ...
  const handleExportExcel = async () => {
    ...
    utils.book_append_sheet(wb, ws, "ThanhVien");
    writeFile(wb, "Danh_sach_thanh_vien.xlsx");
  };
  ...
  const handleReactivateAllSso = async () => {
    const ok = await confirm({
      title: "Kích hoạt lại tất cả thành viên có trong SSO?",
      description:
        "Thao tác này sẽ đặt lại trạng thái Hoạt động cho tất cả thành viên đang bị đánh dấu Khóa nhưng vẫn còn trong danh sách SSO (khắc phục lỗi cũ do đồng bộ nhầm từ cờ Alumni). Thành viên không còn trong SSO sẽ không bị ảnh hưởng.",
      variant: "default",
    ...
    await handleErrorClient({
      cb: () => adminReactivateAllSsoMembers(),
      onSuccess: ({ data }) => {
        const count = (data.payload as { count: number } | undefined)?.count ?? 0;
        toast({
          title: "Hoàn tất",
          description: `Đã kích hoạt lại ${count} thành viên.`,
          variant: "success"
        });
        router.refresh();
      },
        ...
        <div className='flex items-center gap-2'>
          <DropdownMenu>
            ...
            <DropdownMenuContent align='end' className='w-44'>
              ...
              <DropdownMenuItem disabled={isReactivating} onClick={handleReactivateAllSso}>
                <UserCheck className='mr-2 h-4 w-4' />
                Kích hoạt lại (theo SSO)
              </DropdownMenuItem>
