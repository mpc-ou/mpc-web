    ...
    <Dialog onOpenChange={onOpenChange} open={open}>
      <DialogContent className='max-h-[90vh] max-w-3xl overflow-y-auto'>
        ...
        <div className='grid grid-cols-1 gap-x-6 gap-y-4 text-sm md:grid-cols-2'>
          {data.map((f) => (
            <div className={`flex flex-col gap-1.5 ${f.colSpan === 2 ? "md:col-span-2" : ""}`} key={f.label}>
              <span className='font-medium text-muted-foreground'>{f.label}</span>
              <div className='w-full overflow-hidden break-words rounded-md border bg-muted/30 p-3 text-foreground'>
