...
const FaqSection = async ({ locale, target = "GENERAL" }: { locale: string; target?: string }) => {
        ...
        <Accordion className='w-full space-y-3' collapsible type='single'>
          {items.map((item, idx) => (
            <ScrollReveal delay={idx * 80} key={item.id} variant='fade-up'>
              <AccordionItem className='rounded-xl border border-border bg-background px-5 shadow-sm' value={item.id}>
