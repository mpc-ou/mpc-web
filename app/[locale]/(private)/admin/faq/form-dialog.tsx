    ...
    <Dialog key={faq?.id ?? "new"} onOpenChange={onOpenChange} open={open}>
      <DialogContent className='max-w-6xl'>
        <DialogHeader>
          ...
          <DialogDescription>
            {isEdit ? "Cập nhật câu hỏi song ngữ (Tiếng Việt & Tiếng Anh)." : "Thêm câu hỏi thường gặp mới (song ngữ)."}
          </DialogDescription>
        </DialogHeader>
        <form className='grid gap-4 py-4' id='faq-form' onSubmit={handleSubmit}>
          ...
          <div className='grid gap-2'>
            ...
            <Input defaultValue={faq?.order ?? 0} id='order' name='order' placeholder='0' type='number' />
          </div>
        </form>
        <DialogFooter>
          <Button disabled={loading} form='faq-form' type='submit'>
            {loading ? "Đang lưu..." : isEdit ? "Lưu thay đổi" : "Thêm"}
            {/* biome-ignore lint/style/noNestedTernary: clear intent */}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
