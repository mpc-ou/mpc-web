...
import {
  LanguageToggle,
  type ViewLanguage,
} from "@/components/custom/language-toggle";
...
export default function ProjectForm({ project, allMembers = [] }: Props) {
  ...
  const [imagesItems, setImagesItems] = useState<ImageItem[]>(
    ((project as { images?: string[] } | null | undefined)?.images ?? []).map(
      (url) => ({ url }),
    ),
  );
  ...
  const processThumbnailFile = async (file: File) => {
    ...
    try {
      ...
      toast({
        variant: "destructive",
        description:
          err instanceof Error ? err.message : "Upload thumbnail thất bại",
      });
    } finally {
  ...
  const persistProject = async (
    payload: Parameters<typeof adminCreateProject>[0],
  ) => {
    if (isEdit && project) {
      return {
  ...
  const handleTranslate = (result: {
    ...
    if (result.title !== undefined) {
      otherLang === "vi"
        ? bi.setTitleVi(result.title)
        : bi.setTitleEn(result.title);
    }
    if (result.summary !== undefined) {
      otherLang === "vi"
        ? bi.setSummaryVi(result.summary)
        : bi.setSummaryEn(result.summary);
    }
    if (result.content !== undefined) {
      otherLang === "vi"
        ? bi.setContentVi(result.content)
        : bi.setContentEn(result.content);
    }
  };
  ...
  const setTitleField = (v: string) =>
    bi.viewLang === "vi" ? bi.setTitleVi(v) : bi.setTitleEn(v);
  const getDescription = () =>
    bi.viewLang === "vi" ? (bi.summaryVi ?? "") : (bi.summaryEn ?? "");
  const setDescriptionField = (v: string) =>
    bi.viewLang === "vi"
      ? bi.setSummaryVi(v || null)
      : bi.setSummaryEn(v || null);
          ...
          <div>
            ...
            <p className="text-muted-foreground text-xs">
              {isEdit
                ...
                : "Tạo một dự án nghiên cứu hoặc sản phẩm mới"}
            </p>
          </div>
        </div>
        <Button
        ...
        >
          ...
          <Card>
            <CardHeader className="py-3">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <TranslateButton
                  disabled={
                    !(translateSrcFields.title || translateSrcFields.content)
                  }
          ...
          <Card>
            ...
            <CardContent className="grid gap-4">
              <MarkdownEditor
                defaultValue={
                  bi.viewLang === "vi" ? bi.contentVi : bi.contentEn
                }
                key={bi.viewLang}
                minHeight="240px"
                name="content"
                onChange={(v) =>
                  bi.viewLang === "vi" ? bi.setContentVi(v) : bi.setContentEn(v)
                }
        ...
        <div className="sticky top-6 hidden max-h-[calc(100vh-80px)] flex-col gap-4 overflow-y-auto rounded-xl border bg-background/50 p-6 backdrop-blur-md lg:flex">
          ...
          {pickLang(bi.viewLang, bi.contentVi, bi.contentEn) ? (
            <div className="border-t pt-4">
              <MarkdownContent
                content={pickLang(bi.viewLang, bi.contentVi, bi.contentEn)}
              />
            </div>
          ) : (
