...
export function LeadershipTimelineClient({ roles, locale }: { roles: HistoricalRole[]; locale: string }) {
  ...
  return (
    <>
      <Button className='rounded-full shadow-lg' onClick={() => setOpen(true)} size='lg'>
        <History className='mr-2 h-5 w-5' /> {t("button")}
      </Button>

      <Dialog onOpenChange={setOpen} open={open}>
        <DialogContent className='max-h-[90vh] overflow-y-auto sm:max-w-4xl'>
          <DialogHeader>
            <DialogTitle className='text-center font-bold text-2xl'>{t("dialogTitle")}</DialogTitle>
          </DialogHeader>
