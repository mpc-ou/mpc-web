...
import type { UserProfileData } from "@/components/custom/header/user-profile.client";
...
const PageLayout = async ({ children }: { children: ReactNode }) => {
  ...
  const [announcementResult, profileResult, footerResult, brandingResult] = await Promise.allSettled([
    ...
    prisma.siteSetting.findMany({
      where: {
        key: {
          in: ["site_title", "site_logo", "site_primary_color"]
        }
      }
    })
  ]);
  ...
  const brandingRes = brandingResult.status === "fulfilled" ? brandingResult.value : [];
  const brandingMap: Record<string, string> = {};
  if (Array.isArray(brandingRes)) {
    for (const s of brandingRes) {
