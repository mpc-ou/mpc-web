...
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: MemberForm manages complex state
export function MemberForm({ member, departments }: Props) {
  ...
  const [showDob, setShowDob] = useState(member?.showDob ?? false);
  const [showPhone, setShowPhone] = useState(member?.showPhone ?? false);
  const [showStudentId, setShowStudentId] = useState(member?.showStudentId ?? false);
  ...
  // Roles updates
  const _handlePositionChange = (val: string) => {
    setRolePosition(val);
    const requiresDept =
  ...
  const _handleEditRoleClick = (role: ClubRoleEntry) => {
    setEditingRole(role);
  ...
  const _handleRoleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!rolePosition) {
  ...
  const _handleRemoveRole = async (roleId: string) => {
  ...
  // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: handleMainSave validates and updates member transaction
      ...
      <div className='grid grid-cols-1 gap-6 lg:grid-cols-2'>
        ...
        <div className='flex flex-col gap-5 rounded-xl border bg-background p-6 shadow-sm'>
          ...
          <div className='grid gap-4'>
            ...
            <div className='flex items-center gap-2 rounded-lg border border-border bg-muted/10 p-3'>
              <Checkbox checked={isActive} id='isActive' onCheckedChange={(c) => setIsActive(!!c)} />
              <div>
                <Label className='cursor-pointer select-none font-medium text-sm' htmlFor='isActive'>
                  Thành viên đang hoạt động
                </Label>
                <p className='text-[10px] text-muted-foreground'>
                  Đặt thủ công. Có thể bị SSO ghi đè khi đồng bộ (thành viên rời CLB / không còn trong SSO).
                </p>
              </div>
            </div>
          </div>
        ...
        <div className='flex flex-col gap-6'>
          ...
          <div className='flex flex-col gap-4 rounded-xl border bg-background p-6 shadow-sm'>
            <h2 className='flex items-center gap-2 font-semibold text-base text-foreground'>
              <LinkIcon className='h-4 w-4 text-primary' /> Mạng xã hội
            </h2>
