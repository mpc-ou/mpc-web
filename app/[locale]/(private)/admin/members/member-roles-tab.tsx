...
import { adminAddClubRole, adminGetMemberClubRoles, adminRemoveClubRole } from "../actions";
...
export function MemberRolesTab({ member, departments, open }: Props) {
  ...
  const handleAddRole = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await adminAddClubRole(member.id, {
      ...
      departmentId: departmentId === "none" ? undefined : departmentId || undefined,
    ...
    <>
      ...
      <div className='space-y-4 pt-2'>
        <div className='flex items-center justify-between'>
          ...
          <Button onClick={() => setShowRoleForm((v) => !v)} size='sm' type='button' variant='outline'>
        ...
        {showRoleForm && (
          ...
          >
            <p className='font-medium text-primary text-xs uppercase tracking-wide'>Thêm chức vụ mới</p>
            ...
            <div className='grid grid-cols-3 gap-3'>
              <div className='grid gap-1.5'>
                ...
                <Input min={2010} name='term' placeholder='VD: 2024' type='number' />
              </div>
              <div className='grid gap-1.5'>
                <Label>Bắt đầu *</Label>
            ...
            <div className='flex justify-end gap-2'>
              <Button onClick={() => setShowRoleForm(false)} size='sm' type='button' variant='ghost'>
                Huỷ
              </Button>
              <Button disabled={addingRole} size='sm' type='submit'>
                {addingRole ? <Loader2 className='h-3 w-3 animate-spin' /> : "Thêm"}
              </Button>
            </div>
          </form>
        )}
        ...
        {rolesLoading ? (
          ...
          <p className='py-6 text-center text-muted-foreground text-sm'>Chưa có lịch sử chức vụ nào</p>
        ) : (
                  ...
                  <div className='flex flex-1 items-start justify-between gap-2'>
                    <div>
                      <div className='flex items-center gap-2'>
                        <span className='font-medium text-sm'>{POSITION_LABELS[role.position] ?? role.position}</span>
                        {isActive && (
                      ...
                      {role.department && (
                        <span className='text-muted-foreground text-xs'>📌 {role.department.nameVi}</span>
                      )}
                      <div className='mt-0.5 text-muted-foreground text-xs'>
                        {new Date(role.startAt).toLocaleDateString("vi-VN")}
                        {" → "}
                        {role.endAt ? new Date(role.endAt).toLocaleDateString("vi-VN") : "nay"}
                      </div>
                      {role.note && <p className='mt-0.5 text-muted-foreground text-xs italic'>{role.note}</p>}
                    </div>
                    <Button
