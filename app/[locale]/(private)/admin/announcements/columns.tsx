...
export const createColumns = (
...
): ColumnDef<AnnouncementRow>[] => [
  {
    ...
    cell: ({ row }) => (
      <p className="max-w-xs truncate text-sm">{row.original.content}</p>
    ),
  },
  {
    id: "link",
    ...
    cell: ({ row }) => {
      ...
      if (!linkUrl) return <span className="text-muted-foreground">—</span>;
      return (
      ...
      return (
        <div className="flex items-center gap-2">
          <div
            ...
            title={bg}
          />
          <span className="max-w-25 truncate text-muted-foreground text-xs">
            {bg}
          </span>
        </div>
      );
    ...
    cell: ({ row }) =>
      row.original.endAt ? (
        <span className="text-xs">
          {new Date(row.original.endAt).toLocaleDateString("vi-VN")}
        </span>
      ) : (
