...
export function GoldBoard({ members, locale }: { members: GoldBoardMember[]; locale: string }) {
  ...
  const sorted = [...members]
    .filter((m) => (m.member._count?.achievementEntries ?? 0) > 0)
    .sort((a, b) => (b.member._count?.achievementEntries ?? 0) - (a.member._count?.achievementEntries ?? 0));
...
function GoldCard({ item, locale, onClick }: { item: GoldBoardMember; locale: string; onClick: () => void }) {
  const fullName = getFullName(item.member.firstName, item.member.middleName, item.member.lastName, locale);
